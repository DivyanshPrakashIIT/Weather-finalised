...
<style>
    ...
    /* Apply font globally - updated selector for Streamlit 1.35+ */
    html, body, * {
    ...
    /* Slider track - updated for Streamlit 1.35+ */
    [data-testid="stSlider"] [role="slider"] { background: #2E6DA4 !important; }
    [data-testid="stSlider"] > div > div { background: transparent !important; }
    [data-testid="stSlider"] input[type="range"]::-webkit-slider-runnable-track { background: #1E3A5F !important; }
    [data-testid="stSlider"] div[data-baseweb="slider"] div { background: #1E3A5F; }

    /* Button - updated selectors for Streamlit 1.35+ */
    ...
    [data-testid="stBaseButton-primary"]:hover { opacity: 0.9; }

    /* Markdown text */
    [data-testid="stMarkdownContainer"] p { color: #9BBCDA; }
    [data-testid="stMarkdown"] p { color: #9BBCDA; }

    /* Headings */
    h1, h2, h3 { color: #E8F4FD !important; }

    /* Tabs - updated for Streamlit 1.35+ */
    ...
    /* Metrics - updated for Streamlit 1.35+ */
    ...
    [data-testid="stMetricLabel"] p { color: #6B9EC7 !important; font-size: 0.75rem !important; }
    ...
    [data-testid="stNumberInputContainer"] { background: #0A1520 !important; border-color: #1E3A5F !important; }
...
def run_predictions(temp, humidity, wind, pressure, lag1, lag2):
    ...
    # ── ARIMA / SARIMA bias correction ──────────────────────────────────────
    # These statistical models are trained on historical data and tend to
    # predict near the long-run seasonal mean (~11–13 °C for Delhi winter
    # patterns) rather than the current ambient temperature.  XGBoost and
    # LightGBM, which use rich lag/rolling features, are far closer to the
    # true current temperature (≈30 °C today).  We anchor ARIMA/SARIMA to
    # the ML-model mean so they contribute trend signal rather than a large
    # absolute bias.
    ml_preds = [preds[m] for m in ('XGBoost', 'LightGBM') if m in preds]
    if ml_preds and ('ARIMA' in preds or 'SARIMA' in preds):
        ml_mean = float(np.mean(ml_preds))
        for stat_model in ('ARIMA', 'SARIMA'):
            if stat_model in preds:
                raw = preds[stat_model]
                # Blend: keep the statistical model's deviation direction but
                # re-centre it around the ML mean so the absolute level is
                # realistic.  Weight: 70 % ML anchor + 30 % raw stat forecast.
                preds[stat_model] = round(0.70 * ml_mean + 0.30 * raw, 2)

    # Ensemble — new fixed weights: XGB 25 %, LGB 25 %, LSTM 20 %,
    #            ARIMA 15 %, SARIMA 15 %
    if len(preds) >= 2:
        BASE_W = {'XGBoost': 0.25, 'LightGBM': 0.25,
                  'LSTM': 0.20, 'ARIMA': 0.15, 'SARIMA': 0.15}
        # "Strong" tier models absorb any missing weight (LSTM absent → share goes to XGB+LGB)
...
with tab1:
    if not predict_btn:
        ...
        if not ml_ready:
            ...
            st.markdown('<div class="section-header">Tomorrow\'s Temperature Forecast — All Models</div>',
                        unsafe_allow_html=True)

            # ── Helper: single prediction card ──────────────────────────────
            def model_card(mname, val, extra_badge=''):
                color = temp_color(val)
                emoji = temp_emoji(val)
                return f"""
                <div class="pred-card" style="height:100%;">
            ...
            avail_ml    = [m for m in ml_models   if m in preds]
            avail_stat  = [m for m in stat_models if m in preds]

            if avail_ml:
                st.markdown("""<div style="font-size:0.7rem;font-weight:700;letter-spacing:0.1em;
                    text-transform:uppercase;color:#27AE60;margin:0.8rem 0 0.5rem 0;">
                    🤖 Machine Learning Models</div>""", unsafe_allow_html=True)
                ml_cols = st.columns(len(avail_ml))
                for col, mname in zip(ml_cols, avail_ml):
                    with col:
                        st.markdown(model_card(mname, preds[mname]), unsafe_allow_html=True)

            # ── Row 2: Statistical models (bias-corrected) ───────────────────
            if avail_stat:
                st.markdown("""<div style="font-size:0.7rem;font-weight:700;letter-spacing:0.1em;
                    text-transform:uppercase;color:#8E44AD;margin:1.1rem 0 0.5rem 0;">
                    📈 Statistical Models
                    <span style="font-size:0.62rem;color:#5B7FA6;font-weight:400;
                        text-transform:none;letter-spacing:0;margin-left:0.4rem;">
                        (seasonally re-anchored to current temperature level)
                    </span></div>""", unsafe_allow_html=True)
                stat_cols = st.columns(len(avail_stat))
                for col, mname in zip(stat_cols, avail_stat):
                    with col:
                        st.markdown(
                            model_card(mname, preds[mname],
                                       '<span class="pred-badge badge-na">📐 Calibrated</span>'),
                            unsafe_allow_html=True)

            # ── Ensemble hero card ───────────────────────────────────────────
            if 'Ensemble' in preds:
                ens_val   = preds['Ensemble']
                ens_color = temp_color(ens_val)
                ens_emoji = temp_emoji(ens_val)
                ordered_w = ['XGBoost','LightGBM','LSTM','ARIMA','SARIMA']
                w_lines   = '<br>'.join(
                    f'{m}: <b>{weights.get(m,0)*100:.0f}%</b>'
                    for m in ordered_w if m in weights
                )
                st.markdown(f"""
                <div style="margin-top:1.3rem;">
                  <div style="font-size:0.7rem;font-weight:700;letter-spacing:0.1em;
                      text-transform:uppercase;color:#F39C12;margin-bottom:0.5rem;">
                      ★ Final Ensemble Prediction</div>
                  <div class="pred-card ensemble"
                       style="display:flex;align-items:center;justify-content:space-between;
                              flex-wrap:wrap;gap:1.5rem;padding:1.8rem 2rem;">
                    <div>
                      <div class="pred-model-name">Weighted Ensemble</div>
                      <div class="pred-value" style="color:{ens_color};font-size:3.2rem;">{ens_val:.1f}°C</div>
                      <div class="pred-emoji" style="font-size:1.1rem;margin-top:0.3rem;">{ens_emoji}</div>
                    </div>
                    <div style="text-align:right;">
                      <div style="font-size:0.68rem;color:#5B8AB0;font-weight:700;
                          letter-spacing:0.08em;text-transform:uppercase;margin-bottom:0.5rem;">
                          Weight Distribution</div>
                      <div style="font-size:0.85rem;color:#C8DCEF;
            ...
            plot_order = ['XGBoost','LightGBM','LSTM','ARIMA','SARIMA','Ensemble']
            names = [n for n in plot_order if n in preds]
            vals  = [preds[n] for n in names]
            COLOR_MAP = {
                'XGBoost':  '#3498DB',  # blue  — ML
                'LightGBM': '#2ECC71',  # green — ML
                'LSTM':     '#1ABC9C',  # teal  — ML
                'ARIMA':    '#9B59B6',  # purple — statistical
                'SARIMA':   '#8E44AD',  # purple — statistical
                'Ensemble': '#F39C12',  # gold  — ensemble
            }
            colors = [COLOR_MAP.get(n, '#3A7BBF') for n in names]
            ...
            ax.axhline(ensemble_val, color='#F39C12', lw=1.2,
                       ls='--', alpha=0.4, label=f'Ensemble ({ensemble_val:.1f}°C)')
            ax.set_ylabel('°C', color='#6B9EC7', fontsize=10)
            ax.tick_params(colors='#6B9EC7', labelsize=9)
            for spine in ax.spines.values():
                spine.set_edgecolor('#1E3A5F')
            ax.set_title("Tomorrow's Temperature — All Model Predictions",
                         color='#9BBCDA', fontsize=10, pad=8)
            # Colour legend
            from matplotlib.patches import Patch
            legend_els = [
                Patch(facecolor='#3498DB', label='ML (XGB/LGB)'),
                Patch(facecolor='#1ABC9C', label='LSTM'),
                Patch(facecolor='#9B59B6', label='Statistical'),
                Patch(facecolor='#F39C12', label='Ensemble'),
            ]
            ax.legend(handles=legend_els, fontsize=8, facecolor='#0D1B2A',
                      labelcolor='#9BBCDA', edgecolor='#1E3A5F', loc='lower right')
            plt.tight_layout()
            st.pyplot(fig)
            plt.close()
...
with tab2:
    ...
    if history is None:
        ...
        for _, row in metrics_df.iterrows():
            ...
            table_html += f'<td>{row["MAPE"]:.2f}%</td>'
        ...
        for ax, col, color in zip(axes, pred_cols, palette):
            ...
            ax.set_title(
                ...
                f'MAE={row["MAE"]:.3f}°C  MAPE={row["MAPE"]:.2f}%  R²={row["R²"]:.4f}',
...
with tab3:
    ...
    with c1:
        for title, body in [
            ...
            ("Ensemble Strategy", "Weighted average: XGB 25% · LGB 25% · LSTM 20% · ARIMA 15% · SARIMA 15%\nARIMA/SARIMA are bias-corrected (70% ML anchor + 30% raw) before weighting.\nIf LSTM missing → its 20% redistributed to XGB+LGB equally.\nWeak models never receive bonus weight from absent strong models."),
        ]:
            st.markdown(f"""
            <div class="info-row">
    ...
    st.markdown('<div class="section-header">Base Ensemble Weights (Updated)</div>',
                unsafe_allow_html=True)
    weight_data = {
        'Model':   ['XGBoost', 'LightGBM', 'LSTM', 'ARIMA', 'SARIMA'],
        'Weight':  ['0.250',   '0.250',    '0.200', '0.150', '0.150'],
        'Tier':    ['Strong',  'Strong',   'Strong','Weak ★ Calibrated',  'Weak ★ Calibrated'],
        'Note':    ['Lag+rolling features → excellent seasonal tracking',
                    'Lag+rolling features → excellent seasonal tracking',
                    'BiLSTM+Attention 30-day lookback',
                    'Re-anchored: 70% ML mean + 30% raw forecast',
                    'Re-anchored: 70% ML mean + 30% raw forecast'],
    ...
    <div class="info-row" style="margin-top:0.8rem;">
      <div class="info-label">⚠️ Why ARIMA/SARIMA are calibrated</div>
      <div class="info-value">These statistical models forecast near the long-run historical mean
      (~11–13 °C) rather than the current season's temperature, causing ~18 °C bias in summer.
      The calibration step re-centres their output: <b>calibrated = 0.70 × ML_mean + 0.30 × raw_stat</b>,
      preserving the model's trend direction while correcting the absolute level.
      XGBoost and LightGBM (which use 7-day lag features) accurately track today's
      30 °C conditions.</div>
    </div>
    """, unsafe_allow_html=True)
